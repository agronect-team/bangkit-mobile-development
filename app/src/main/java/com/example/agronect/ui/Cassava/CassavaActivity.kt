...
import com.example.agronect.databinding.ActivityAddCassavaBinding
...
class CassavaActivity : AppCompatActivity(){
    private lateinit var binding: ActivityAddCassavaBinding

    private val viewModel by viewModels<CassavaViewModel> {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding = ActivityAddCassavaBinding.inflate(layoutInflater)
        ...
        currentImageUri?.let { uri ->
            ...
            lifecycleScope.launch {
                try {
                    ...
                    val call = apiService.uploadCassava("Bearer $token", imagePart)

                    call.enqueue(object : Callback<CassavaResponse> {
                        override fun onResponse(call: Call<CassavaResponse>, response: Response<CassavaResponse>) {
                            showLoading(false)
                            if (response.isSuccessful) {
                                response.body()?.let { CassavaResponse ->
                                    Log.e(ContentValues.TAG, "onSuccess: ${response.message()}")
                                    showToast("Upload successful")
                                    navigateToResultActivity(CassavaResponse)
                                }
                            } else {
    ...
    private fun navigateToResultActivity(cassavaResponse: CassavaResponse) {
        val intent = Intent(this, AllClassificationResultActivity::class.java).apply {
            putExtra("imageUri", currentImageUri.toString())
            putExtra("confidence", cassavaResponse.confidence.toString())
            putExtra("predictions", cassavaResponse.prediction)
            putExtra("description", cassavaResponse.description)
            putExtra("solution", cassavaResponse.solution)
            putExtra("plantName", cassavaResponse.plantName)
        }
