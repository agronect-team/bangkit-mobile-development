...
import com.example.agronect.databinding.ActivityAddRiceBinding
...
class AddRiceActivity : AppCompatActivity(){
    private lateinit var binding: ActivityAddRiceBinding

    private val viewModel by viewModels<RiceViewModel> {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding = ActivityAddRiceBinding.inflate(layoutInflater)
        ...
        currentImageUri?.let { uri ->
            ...
            lifecycleScope.launch {
                try {
                    ...
                    val call = apiService.uploadRice("Bearer $token", imagePart)

                    call.enqueue(object : Callback<RiceResponse> {
                        override fun onResponse(call: Call<RiceResponse>, response: Response<RiceResponse>) {
                            showLoading(false)
                            if (response.isSuccessful) {
                                response.body()?.let { RiceResponse ->
                                    Log.e(ContentValues.TAG, "onSuccess: ${response.message()}")
                                    showToast("Upload successful")
                                    navigateToResultActivity(RiceResponse)
                                }
                            } else {
                        ...
                        override fun onFailure(call: Call<RiceResponse>, t: Throwable) {
                            showLoading(false)
    ...
    private fun navigateToResultActivity(riceResponse: RiceResponse) {
        val intent = Intent(this, AllClassificationResultActivity::class.java).apply {
            putExtra("imageUri", currentImageUri.toString())
            putExtra("confidence", riceResponse.confidence.toString())
            putExtra("predictions", riceResponse.prediction)
            putExtra("description", riceResponse.description)
            putExtra("solution", riceResponse.solution)
            putExtra("plantName", riceResponse.plantName)
        }
