...
import com.example.agronect.databinding.ActivityAddTomatoBinding
...
class AddTomatoActivity : AppCompatActivity(){
    private lateinit var binding: ActivityAddTomatoBinding

    private val viewModel by viewModels<TomatoViewModel> {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding = ActivityAddTomatoBinding.inflate(layoutInflater)
        ...
        currentImageUri?.let { uri ->
            ...
            lifecycleScope.launch {
                try {
                    ...
                    val call = apiService.uploadTomato("Bearer $token", imagePart)
                    ...
                    call.enqueue(object : Callback<TomatoResponse> {
                        override fun onResponse(call: Call<TomatoResponse>, response: Response<TomatoResponse>) {
                            ...
                            if (response.isSuccessful) {
                                response.body()?.let { TomatoResponse ->
                                    Log.e(ContentValues.TAG, "onSuccess: ${response.message()}")
                                    showToast("Upload successful")
                                    navigateToResultActivity(TomatoResponse)
                                }
                            } else {
    ...
    private fun navigateToResultActivity(tomatoResponse: TomatoResponse) {
        val intent = Intent(this, AllClassificationResultActivity::class.java).apply {
            putExtra("imageUri", currentImageUri.toString())
            putExtra("confidence", tomatoResponse.confidence.toString())
            putExtra("predictions", tomatoResponse.prediction)
            putExtra("description", tomatoResponse.description)
            putExtra("solution", tomatoResponse.solution)
            putExtra("plantName", tomatoResponse.plantName)
        }
