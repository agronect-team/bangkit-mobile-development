...
class OrangeActivity : AppCompatActivity(){
    ...
    private val viewModel by viewModels<OrangeViewModel> {
        ...
        currentImageUri?.let { uri ->
            ...
            lifecycleScope.launch {
                try {
                    ...
                    val call = apiService.uploadOrange("Bearer $token", imagePart)

                    call.enqueue(object : Callback<OrangeResponse> {
                        override fun onResponse(call: Call<OrangeResponse>, response: Response<OrangeResponse>) {
                            showLoading(false)
                            if (response.isSuccessful) {
                                response.body()?.let { OrangeResponse ->
                                    Log.e(ContentValues.TAG, "onSuccess: ${response.message()}")
                                    showToast("Upload successful")
                                    navigateToResultActivity(OrangeResponse)
                                }
                            } else {
                        ...
                        override fun onFailure(call: Call<OrangeResponse>, t: Throwable) {
                            showLoading(false)
    ...
    private fun navigateToResultActivity(orangeResponse: OrangeResponse) {
        val intent = Intent(this, AllClassificationResultActivity::class.java).apply {
            putExtra("imageUri", currentImageUri.toString())
            putExtra("confidence", orangeResponse.confidence.toString())
            putExtra("predictions", orangeResponse.prediction)
            putExtra("description", orangeResponse.description)
            putExtra("solution", orangeResponse.solution)
            putExtra("plantName", orangeResponse.plantName)
        }
