...
import com.example.agronect.databinding.ActivityAddCornBinding
...
class AddBananaActivity : AppCompatActivity(){
    private lateinit var binding: ActivityAddCornBinding

    private val viewModel by viewModels<BananaViewModel> {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding = ActivityAddCornBinding.inflate(layoutInflater)
        ...
        currentImageUri?.let { uri ->
            ...
            lifecycleScope.launch {
                try {
                    ...
                    val call = apiService.uploadBanana("Bearer $token", imagePart)

                    call.enqueue(object : Callback<BananaResponse> {
                        override fun onResponse(call: Call<BananaResponse>, response: Response<BananaResponse>) {
                            showLoading(false)
                            if (response.isSuccessful) {
                                response.body()?.let { BananaResponse ->
                                    Log.e(ContentValues.TAG, "onSuccess: ${response.message()}")
                                    showToast("Upload successful")
                                    navigateToResultActivity(BananaResponse)
                                }
                            } else {
                        ...
                        override fun onFailure(call: Call<BananaResponse>, t: Throwable) {
                            showLoading(false)
    ...
    private fun navigateToResultActivity(bananaResponse: BananaResponse) {
        val intent = Intent(this, AllClassificationResultActivity::class.java).apply {
            putExtra("imageUri", currentImageUri.toString())
            putExtra("confidence", bananaResponse.confidence.toString())
            putExtra("predictions", bananaResponse.prediction)
            putExtra("description", bananaResponse.description)
            putExtra("solution", bananaResponse.solution)
            putExtra("plantName", bananaResponse.plantName)
        }
