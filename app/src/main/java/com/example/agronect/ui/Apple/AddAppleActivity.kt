package com.example.agronect.ui.Apple
...
import com.example.agronect.databinding.ActivityAddAppleBinding
...
class AddAppleActivity : AppCompatActivity(){
    private lateinit var binding: ActivityAddAppleBinding

    private val viewModel by viewModels<AppleViewModel> {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding = ActivityAddAppleBinding.inflate(layoutInflater)
        ...
        currentImageUri?.let { uri ->
            ...
            lifecycleScope.launch {
                try {
                    ...
                    val call = apiService.uploadApple("Bearer $token", imagePart)

                    call.enqueue(object : Callback<AppleResponse> {
                        override fun onResponse(call: Call<AppleResponse>, response: Response<AppleResponse>) {
                            showLoading(false)
                            if (response.isSuccessful) {
                                response.body()?.let { AppleResponse ->
                                    Log.e(ContentValues.TAG, "onSuccess: ${response.message()}")
                                    showToast("Upload successful")
                                    navigateToResultActivity(AppleResponse)
                                }
                            } else {
    ...
    private fun navigateToResultActivity(appleResponse: AppleResponse) {
        val intent = Intent(this, AllClassificationResultActivity::class.java).apply {
            putExtra("imageUri", currentImageUri.toString())
            putExtra("confidence", appleResponse.confidence.toString())
            putExtra("predictions", appleResponse.prediction)
            putExtra("description", appleResponse.description)
            putExtra("solution", appleResponse.solution)
            putExtra("plantName", appleResponse.plantName)
        }
