...
class CornActivity : AppCompatActivity(){
    ...
    private val viewModel by viewModels<CornViewModel> {
        ...
        currentImageUri?.let { uri ->
            ...
            lifecycleScope.launch {
                try {
                    ...
                    val call = apiService.uploadCorn("Bearer $token", imagePart)

                    call.enqueue(object : Callback<CornResponse> {
                        override fun onResponse(call: Call<CornResponse>, response: Response<CornResponse>) {
                            showLoading(false)
                            if (response.isSuccessful) {
                                response.body()?.let { cornResponse ->
                                    Log.e(ContentValues.TAG, "onSuccess: ${response.message()}")
                                    showToast("Upload successful")
                                    navigateToResultActivity(cornResponse)
                                }
                            } else {
    ...
    private fun navigateToResultActivity(cornResponse: CornResponse) {
        val intent = Intent(this, AllClassificationResultActivity::class.java).apply {
            putExtra("imageUri", currentImageUri.toString())
            putExtra("confidence", cornResponse.confidence.toString())
            putExtra("predictions", cornResponse.prediction)
            putExtra("description", cornResponse.description)
            putExtra("solution", cornResponse.solution)
            putExtra("plantName", cornResponse.plantName)
        }
